...
import com.ioannapergamali.mysmartroute.data.local.FavoriteRouteDao
    ...
    companion object {
        ...
        private val MIGRATION_15_16 = object : Migration(15, 16) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                insertOption("opt_admin_5", adminMenuId, "view_unassigned", "viewUnassigned")
                insertOption("opt_admin_6", adminMenuId, "review_poi", "reviewPoi")
                insertOption("opt_admin_7", adminMenuId, "rank_drivers", "rankDrivers")
        ...
        private val MIGRATION_55_56 = object : Migration(55, 56) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "INSERT OR IGNORE INTO `menu_options` (`id`, `menuId`, `titleResKey`, `route`) VALUES ('opt_admin_13', 'menu_admin_main', 'view_pois', 'viewPois')"
                )
            }
        }
        ...
        private val MIGRATION_61_62 = object : Migration(61, 62) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_favorite_routes_userId` ON `favorite_routes` (`userId`)")
                database.execSQL("CREATE INDEX IF NOT EXISTS `index_favorite_routes_routeId` ON `favorite_routes` (`routeId`)")
            }
        }
        ...
        private val MIGRATION_66_67 = object : Migration(66, 67) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                database.execSQL(
                    "INSERT INTO transport_declarations_new (id, routeId, driverId, cost, durationMinutes, date, startTime) " +
                        "SELECT id, routeId, driverId, cost, durationMinutes, date, startTime FROM transport_declarations"
                )
                database.execSQL("DROP TABLE transport_declarations")
                database.execSQL("ALTER TABLE transport_declarations_new RENAME TO transport_declarations")
                database.execSQL(
                    "ALTER TABLE `transport_declarations_details` ADD COLUMN `seats` INTEGER NOT NULL DEFAULT 0"
                )
            }
        }
        ...
        private fun prepopulate(db: SupportSQLiteDatabase) {
            ...
            insertOption("opt_admin_12", adminMenuId, "view_pois", "viewPois")
